...
$sellerID = substr(strstr($link, '?'), strlen('?'));
...
if($email_check){
     ...
     $id = $userClass->reportSeller($name, $email, $subject, $message, $sellerID);
...
<div class="content3">
	...
	<!-- Section !-->
	<div class="section">
		<h2><b>REPORT SELLER</b></h2>
		<p>Any complaint? Send us your message here</p>
	</div>
	...
	<div class="mainn">
		...
		$dbConnection = $userClass->DBConnect();
			...
			$result->bindParam(":id", $sellerID ,PDO::PARAM_STR);
			$result->execute();
			$data=$result->fetch(PDO::FETCH_OBJ);
		?>
			<p><?php echo $data->store ?></p>
		</div>
	</div>
